...
#     ),
# ]
# def prepare_eval_dataset(**context):
#     """
#     Prepare the evaluation dataset from the test data
#     """
...
#     # Convert the test JSONL to the evaluation format
#     with open(test_file, 'r') as f:
#         test_data = [json.loads(line) for line in f]
    
#     eval_data = []
#     for item in test_data:
#         # Extract query, context, and response from the JSONL format
#         user_parts = item['contents'][0]['parts'][0]['text']
#         model_response = item['contents'][1]['parts'][0]['text']
        
#         # Parse the instruction and context from the formatted prompt
#         instruction_start = user_parts.find("Question:") + 9
#         context_start = user_parts.find("Context:") + 8
        
#         instruction = user_parts[instruction_start:context_start].strip()
#         context = user_parts[context_start:].strip()
...
#     eval_file = "tmp/eval_dataset.jsonl"
...
#     # Upload to GCS
#     gcs_hook = GCSHook()
#     gcs_path = f"eval_data/eval_dataset_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jsonl"
#     gcs_hook.upload(
...
#     eval_dataset_uri = f"gs://{bucket_name}/{gcs_path}"
#     context['ti'].xcom_push(key='eval_dataset_uri', value=eval_dataset_uri)
#     return eval_dataset_uri
