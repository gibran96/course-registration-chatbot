...
def generate_llm_response(**context):
    ...
    train_data_df = pd.DataFrame(columns=['query', 'context', 'response'])
    for query, response in query_responses.items():
        ...
        for crn, content in zip(crns, content):
            ...
            train_data_df = train_data_df.append({'query': query, 'context': content, 'response': llm_res}, ignore_index=True)
